...
namespace Hypa {
    ...
    void Vulkan::createVertexBuffer(std::vector<Vertex> vertices) {
        ...
        if (vkCreateBuffer(device, &bufferInfo, nullptr, &vertexBuffer) != VK_SUCCESS) {
            throw std::runtime_error("[Hypa::Core::Vulkan] Error: Failed to create vertex buffer!");
        }
        ...
        if (vkAllocateMemory(device, &allocInfo, nullptr, &vertexBufferMemory) != VK_SUCCESS) {
            throw std::runtime_error("[Hypa::Core::Vulkan] Error: Failed to allocate vertex buffer memory!");
        }
