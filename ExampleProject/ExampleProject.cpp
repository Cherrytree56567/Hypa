...
int main() {
	...
	while (app.Update()) {
		if (app.GetEventSystem()->isUnhandledEvent(Drizzle::EventType::WindowMoved)) { ALT = true; }
		if (app.GetEventSystem()->isUnhandledEvent(Drizzle::EventType::WindowMoved) && ALT == true) {
        ...
        if (LLL) {
            ...
            Drizzle::UniformBufferObject uni = app.GetRenderingAPISystem()->GetCurrentRenderingAPI()->GetUniform(app.GetRenderingAPISystem()->GetCurrentRenderingAPI()->GetCurrentShaderName());
